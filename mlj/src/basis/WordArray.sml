...
structure WordArray :> MONO_ARRAY 
  where type elem = word and type vector = WordVector.vector
= struct

open Array

type elem = word
type array = word array
...
fun extract (a, i, slicelen) =
  ...
  in
    WordVector.tabulate(n, fn j => sub(a, Int.+(i,j)))
  end

fun copyVec {src=a1, si=i1, len, dst=a2, di=i2} =
    let val n = 
      case len of Option.NONE => Int.-(WordVector.length a1, i1) 
                | Option.SOME k => k
    in
	if Int.<(n,0) orelse Int.<(i1,0) orelse Int.>(Int.+(i1,n), 
          WordVector.length a1) orelse Int.<(i2,0) orelse Int.>(Int.+(i2,n),
            length a2)
	    then 
		raise General.Subscript
	else 
	    let fun lo2hi j = if Int.<(j, n) then
		(update (a2, Int.+(i2,j), WordVector.sub(a1, Int.+(i1,j))); 
                  lo2hi (Int.+(j,1))) else ()
	    in lo2hi 0 end
    end
